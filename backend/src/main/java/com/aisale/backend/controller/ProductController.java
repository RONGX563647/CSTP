    ...
    @Tag(name = "商品管理", description = "商品 CRUD 及查询接口")
    @RestController
    ...
    public class ProductController {
        ...
        @Operation(summary = "获取在售商品")
        @GetMapping("/on-sale")
        public ApiResponse<Page<ProductResponse>> getOnSaleProducts(
                @RequestParam(defaultValue = "0") int page,
                @RequestParam(defaultValue = "10") int size) {
            Pageable pageable = PageRequest.of(page, size);
        ...
        public ApiResponse<Page<ProductResponse>> searchProducts(
            ...
            Page<ProductResponse> products = productService.searchProducts(
                    name, category, minPrice, maxPrice, status, pageable);
            return ApiResponse.success(products);
        }
