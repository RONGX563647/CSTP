...
graph TB
    subgraph 前端层
        A[Vue 3 前端] -->|HTTP/JSON| B[Vite 开发服务器]
    end
    
    subgraph 后端层
        B -->|代理转发 | C[Spring Boot 3.3.4]
        C --> D[Spring Security]
        C --> E[Controller 层]
        E --> F[Service 层]
        F --> G[Repository 层]
    ...
    subgraph 认证组件
        D --> I[JwtAuthenticationFilter]
        I --> J[JwtUtil]
        D --> K[CustomUserDetailsService]
    end
```
...
graph TB
    ...
    subgraph JwtAuthenticationFilter
